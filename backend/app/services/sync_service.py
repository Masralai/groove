...
class SyncAlreadyRunningError(Exception):
    """Raised when a data sync is attempted while another is in progress."""

    def __init__(self, message: str = "Another sync is already in progress"):
...
class DataSyncService:
    ...
    async def _get_last_sync_date(self) -> datetime | None:
    ...
    async def sync_all(self, full_sync: bool = False) -> dict[str, int]:
        ...
        Acquires a PostgreSQL advisory lock (ID 42) to prevent concurrent syncs.
        If another sync is already running, raises SyncAlreadyRunningError.
        The lock is session-level and auto-releases on connection drop.
        """
        logger.info(f"Starting full data sync (full_sync={full_sync})")

        # Acquire session-level advisory lock to prevent concurrent syncs
        ...
        try:
            ...
            if not result.scalar():
                ...
                raise SyncAlreadyRunningError("Another sync is already in progress")
            ...
            try:
                ...
                await lock_session.execute(text("SELECT pg_advisory_unlock(42)"))
                await lock_session.close()
