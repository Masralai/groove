...
import { computeKPIs, formatCurrency, formatCompact, statusBadgeClass, type InsightRow } from "@/lib/dashboard-utils";
...
export default function DashboardPage() {
  ...
  const [syncResult, setSyncResult] = useState<{ campaigns: number; ad_sets: number; ads: number; insights: number } | null>(null);
  ...
  const dateRanges = [
    ...
    { label: "Last 3 months", value: "3mo", ms: 90 * 24 * 60 * 60 * 1000 },
    { label: "Last 6 months", value: "6mo", ms: 180 * 24 * 60 * 60 * 1000 },
    { label: "Last 12 months", value: "12mo", ms: 365 * 24 * 60 * 60 * 1000 },
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    const tryFetch = async (): Promise<void> => {
      ...
      try {
        ...
        if (selectedRange.ms > 0) {
          const dateFrom = new Date(today.getTime() - selectedRange.ms).toISOString().split('T')[0];
          insightsParams.set("date_from", dateFrom);
          insightsParams.set("date_to", today.toISOString().split('T')[0]);
        }
        const [campaignRes, insightsRes] = await Promise.all([
          fetch(`${API}/api/campaigns?${params}`),
          fetch(`${API}/api/insights?${insightsParams}`),
        ]);
    ...
    await tryFetch();
  }, [statusFilter, offset, selectedRange.ms]);
  ...
  const datePrefix = selectedRange.label.toLowerCase();
  ...
  return (
    <div className="max-w-7xl mx-auto px-10 py-10 space-y-10">
      ...
      {syncResult && (
        <div className="bg-green-500/10 border border-green-500/30 rounded-lg px-5 py-3 text-sm text-green-400 flex items-center space-x-2">
          <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
          </svg>
          <span>
            Synced {syncResult.campaigns} campaign{syncResult.campaigns !== 1 ? "s" : ""},
            {syncResult.ad_sets} ad set{syncResult.ad_sets !== 1 ? "s" : ""},
            {syncResult.ads} ad{syncResult.ads !== 1 ? "s" : ""},
            {syncResult.insights} insight{syncResult.insights !== 1 ? "s" : ""}
          </span>
        </div>
      )}
